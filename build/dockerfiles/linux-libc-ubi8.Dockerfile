...
RUN NODE_ARCH=$(echo "console.log(process.arch)" | node) \
    ...
    node -e "console.log('Max old space size2 : ' + (require('v8').getHeapStatistics().heap_size_limit / 1024 / 1024).toFixed(2) + ' MB')" \
    node -e "console.log('Available CPUs 2: ' + require('os').cpus().length)" \
